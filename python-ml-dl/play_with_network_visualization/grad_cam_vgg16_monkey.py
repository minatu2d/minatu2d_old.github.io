...
def generate_gradcam_at_layer(model, layer_name, n_class = 1000, input_shape=(224,224)):
    ...
    gradcam_fn = "{0}_{1}_gradcam_{2}.jpg".format(PRE_TRAINED_NET, DATASET, layer_name)
    heatmap_fn = "{0}_{1}_heatmap_{2}.jpg".format(PRE_TRAINED_NET, DATASET, layer_name)
...
model = load_model('VGG16_MONKEY_weight_best.hdf5')

# For summary
model.summary()

# Predict for input image
print("Predict class in 10 class of Monkey...")
...
generate_gradcam_at_layer(model, "block5_conv3",
        n_class = n_class, input_shape = (224,224))
